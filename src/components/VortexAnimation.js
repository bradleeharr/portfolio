...
const VortexAnimation = () => {
            ...
            for (let x = 0; x < width; x++) {
                if (grid[y][x]) {
                    if (Math.sin(2*x+y) > 0.5) {
                        ctx.fillStyle = 'rgba(75, 30, 100, 1.0)';
                    }
                    else {
                        ctx.fillStyle = 'rgba(30,75,200,0.9)';
                    }
  ...
  return (
    <div ref={containerRef} id="bg-video" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}></div>
  );
};
